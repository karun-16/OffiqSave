...
function fetchCandidateHeadAndSize(urlStr: string): Promise<any> {
...
function parseDimensions(url: string, w: any, h: any) {
    if (w && h) return { w, h };
...
function getPenalty(url: string) {
    let score = 0;
    ...
    // Penalize known small resolutions even if no resize parameter is explicitly present, 
    // though usually they have resize params.
...
async function run() {
    ...
    function extract(node: any, visited = new Set()) {
    ...
    if (uniqueNodes.length > 0) {
        for (let imgIdx = 0; imgIdx < uniqueNodes.length; imgIdx++) {
            ...
            for (let i = 0; i < candidates.length; i++) {
                ...
                // Scoring Function
                // Base score is Downloaded Bytes.
                // We add penalties (which are negative) for crop/resize.
                let baseScore = net.downloadedBytes || 0;
                let penalty = getPenalty(url);
                let score = baseScore + (penalty * 100); // weight penalty heavily
